...
#        if os.path.isfile(os.path.join(pred_path,'%d_pred.png'%i)):
#            img = cv2.imread(os.path.join(pred_path,'%d_pred.png'%i))
            img = cv2.imread(os.path.join(pred_path, pred_list[i]))
#            test = Image.open(os.path.join(test_path,'%d_test.png'%i))
            test = Image.open(os.path.join(test_path, test_list[i]))
#        else:
#            raise ValueError('Path provided is not a valid file: {}'.format(pred_path))
...
# url = "https://maps.googleapis.com/maps/api/staticmap?center={},{} &zoom=19&size=200x200&maptype=satellite&'.format(center_x, center_y)"
# url_response = urllib.urlopen()
